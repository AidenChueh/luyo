...
## 2026-08-09 06:28
- 範圍：screens/ItineraryScreen.jsx、components/ItinSheet.jsx、components/ConfirmDialog.jsx、styles/global.css、screens/StubScreen.jsx（v1.39）
- 做了什麼：行程規劃主頁與編輯行程 sheet 的 UI/UX 優化。主頁：header 改 .topbar.solid.tight（上下 padding 14/12→10/8），返回鍵拿掉 .ghost 與 add 鍵統一為 .iconbtn、icon 都 20px、陰影降為 0 1px 2px .05；.daypill 加 150-200ms transition，切換日期時用 ref + scrollIntoView({inline:'center'}) 自動捲到可視範圍；當日預估與行程卡金額改用 money(v, trip.sym)（A$269 / 預估 A$269）；timeline 時間欄改成 <b>開始</b> + 細直線 tick + <small>結束</small>，開始 12.5px/700/--ink、結束 11px/500/--muted，tl-dot margin-top 4→5 對齊開始時間，tl-card 左側 accent 4→3px；空狀態改 .itin-empty「今天還沒有行程」＋「＋ 新增第一個行程」文字按鈕；底部按鈕「複製到隔天」改「複製當日行程」並改用新增的 .btn-outline（細框無陰影）flex 1，加行程維持 primary flex 1.3，間距 14→24px；複製改為開 bottom sheet 用 .pick-row 單選目的日（顯示 DAY N · 日期 · 既有行程數）再按「複製行程」，不再自動複製到隔天。編輯 sheet：改用 .sheet.form-sheet.trip-form.itin-form，header 沿用 .sheet-head（高度較原本少約 17%），表單 state 合併成單一 form 物件並保存 base 快照做 dirty 判斷；分類格 .itin-form .cat-cell padding 10→8、gap 6→5、.ci 34→31px（高度約 -10%），border 統一 2px 選中不改尺寸，選中態改品牌色（primary 邊框＋primary-soft 底＋primary 文字），≤340px 改水平捲動；開始/結束加 end<=start 錯誤（紅框＋「結束時間需晚於開始時間，跨日請拆成兩筆行程」）並擋住儲存；備註改 .note-area textarea（min 88px / max 132px，超過內部捲動）；預估與實際花費改用 .money-input 帶 trip.sym 前綴＋groupNum 千分位，state 仍存原始數字；記帳同步提示改 .field-hint（12px muted），存檔成功時 .form-done 顯示「已儲存」並在實際花費 > 0 時加一行「已同步至記帳」；Google Maps 改成三態（未連結顯示「貼上 Google Maps 連結」按鈕／編輯中顯示 URL input＋驗證／已連結顯示「已連結 Google Maps ↗」row 加更換連結與移除），URL 需為 http(s) 且 host 為 google/goo.gl/g.co 否則紅框顯示「請輸入有效的 Google Maps 連結」並擋住儲存；CTA 移除 ✓ icon，editId 時加上 dirty 判斷（沒改就是 disabled 淡橘），刪除鍵與 CTA 間距 10→24px；關閉 sheet（點背景或返回）時若 dirty 改跳確認「尚未儲存變更 / 你有尚未儲存的內容，要離開嗎？」［繼續編輯］［放棄變更］；刪除改為「刪除「X」？/ 刪除後將無法復原。」；ConfirmDialog 新增 confirmIcon 與 cancelText 兩個可選欄位；輸入框 focus 自動 scrollIntoView、Enter 跳下一欄、點空白收鍵盤、金額 decimal / 連結 url keyboard。npm run build 通過
- 為什麼：使用者要求在不重新設計的前提下改善 timeline 時間關係、金額幣別、複製行程彈性，並把偏大的編輯表單壓縮成更成熟的編輯體驗，同時補上 dirty state、未儲存離開確認與刪除二次確認

## 2026-08-09 06:16
- 範圍：screens/TripOverviewScreen.jsx、components/BottomNav.jsx、components/Icon.jsx、lib/dragsort.js、styles/global.css、styles/tokens.css、screens/StubScreen.jsx（v1.38）
- 做了什麼：旅程詳情首頁 UI/UX 精緻化。Hero 日期列合併成單行「8/15–8/21 · 7 天 · 尚未出發」（去掉 icon 與三段 gap），h1 下緣改由 .hero .trip-meta 統一 margin-top 8、font-size 12.5、opacity .9，返回／分享／更多三顆 icon 尺寸統一為 20；天氣卡 padding 14/16→12/14、gap 14→12、icon 34→30、.tmp 28→25px、新增 .cond（13px 600 --ink-2 第二層）與 .sub（11.5px 500 --muted 第三層），高度約 80→71px（-11%），ok / loading / far / nogeo / error 五種狀態合併成同一段結構；快速入口標題列改 .quick-head（baseline 對齊），提示字改「長按拖曳排序」12px 500 muted；.quick padding 14/10→12/8、gap 8→6、.ic 42→39px（radius 13→12）、icon 21→19、.lb 12.5→12px 加 line-height 1.3 與 --ink-2、陰影降為 0 1px 2px .045、press feedback 改 scale(.97)＋--surface-2 底、transition 120ms，卡片高度約 97→87px（-10%）；拖曳把手 icon 從 dots 換成新增的 grip（2×3 六點），顏色由 --line-strong 改 --muted opacity .6；dragsort.js 的 item() 改回傳 data-dragging 與 scale(1.03)/opacity .96/z-index 5，並加 useLayoutEffect FLIP（記錄前後 rect，用 WAAPI 補 180ms 位移）讓其他卡片滑動讓位，排序仍存 localStorage + requestSync；QUICK 陣列改用 hue 對應 tokens 新增的 --feat-orange/blue/green/purple/pink/yellow(-soft) 功能色系（深色模式另給 blue/purple/pink 低亮度底），順序改成 行程｜機票｜住宿 / 清單｜日誌｜相簿 / 記帳｜地點｜地圖；同行者與旅程總結兩張卡抽成共用 .feature-card（padding 14、ic 42px、title 14.5/600、sub 12/500 muted、chevron 18、r-lg、sh-1），高度約 78→72px（-8%），只有 icon 色不同，總結卡完成狀態改用 .feature-card.on 橘框；文案「旅程總結報告卡」→「旅程總結」、「即時彙整花費、地點、日誌」→「即時彙整花費、地點與日誌」；區塊間距改 Hero→天氣 16、天氣→快速入口 24、Grid→同行者 24、同行者→總結 16；--nav-h 58→54（-7%）、nav icon 34→30；.scroll 底部 padding 由 nav-total+28 改 +24。BottomNav 移除 targetOf 的旅程綁定：底部導覽的行程／記帳一律導向全域 /itinerary 與 /expenses，active 也只認全域路徑，旅程內頁（含 /trip/:id/itinerary、/trip/:id/expenses）統一由「旅程」tab 亮起。npm run build 通過
- 為什麼：使用者要求在不重新設計的前提下降低頁面垂直高度、統一元件、把「⋯」換成真正的拖曳把手並補上拖曳回饋，同時釐清旅程內與全域「行程／記帳」的 context

## 2026-08-09 06:04
- 範圍：components/AddTripSheet.jsx、lib/format.js、styles/global.css、screens/StubScreen.jsx（v1.37）
- 做了什麼：新增／編輯旅程表單 UI/UX 優化。Header 加 .sheet.form-sheet（padding-top 10→6、grabber margin 4/14→2/8）與 .sheet-head，標題 15→16px 700，第一個欄位 margin-top 18→14，表頭到「旅程名稱」由約 91px 降到 75px（約 17%）；返回鈕拿掉 .ghost 白底＋陰影，改 --surface-2 平底、維持 40px 圓形與既有 44px 觸控區；.field label 700→600、加 line-height 1.4 與 .opt（可選）淡字樣式；input/select/date-pick 統一 min-height 48px、padding 0 14px、r-md 圓角、--line 邊框、--surface 底、font-size 16px，focus 一律橘框，新增 disabled（--sand 底＋--line-strong 字）與 placeholder（--muted）明確區隔；新增 .field.err 紅框與 .field-err 錯誤訊息（旅程名稱／國家／城市 onBlur 後為空、結束日早於開始日）；國家:城市 由 50/50 改 40/60、主幣別:總預算 由 120px/剩餘 改 33/67，皆用 .form-row（gap 12），≤360px 自動改上下排；日期摘要改 .form-summary（12px、weight 500、muted、icon opacity .7、margin-top 6），狀態字改用 seed 的 STATUS label（規劃中／進行中／已完成）；總預算 label 移除幣別重複，改用 .money-input 把幣別符號放進輸入框左側並隨主幣別更新，輸入即時補千分位（format.js 新增 groupNum，state 仍存原始數字）；封面照片未上傳改 .upload-btn（44px 虛線框、無陰影），已上傳改 .cover-preview（128px、18px 圓角、右上角「更換」與刪除按鈕）；封面色改 .grad-row/.grad-sw，選中用 box-shadow 外框＋check icon（尺寸不變），label 改「預設封面色」並加「未上傳照片時使用」輔助字；CTA 移除 ✓ icon，disabled 改淡橘底＋深橘字（限 .trip-form，其他 sheet 不動），必填改為名稱＋國家＋城市；建立／儲存成功後顯示 .form-done「旅程建立完成」約 0.9 秒再關閉並導向；表單間距統一（label→input 8、input→下一組 20、同列 12、section 24、CTA 24）；輸入框 focus 自動 scrollIntoView 避開鍵盤、Enter 跳下一欄（最後一欄 enterKeyHint=done）、點空白處收鍵盤。npm run build 通過
- 為什麼：使用者要求在不改變品牌設計的前提下讓表單更緊湊專業，統一 Label/Input 樣式與間距、修正欄位比例與預算幣別資訊重複，並補上上傳預覽、錯誤狀態與成功回饋

## 2026-08-08 06:21
- 範圍：screens/HomeScreen.jsx、components/Icon.jsx、styles/global.css、styles/tokens.css、screens/StubScreen.jsx（v1.36）
- 做了什麼：首頁「所有旅程」畫面 UI 優化。Header 加 .topbar.home 變體（上下 padding 10/8、greeting 12.5px、通知鈕 36px），高度由約 66px 降到 54px；搜尋框改 min-height 40px、陰影降到 0 1px 2px rgba(28,25,23,.035)、icon 與 placeholder 垂直置中、左右對齊 .pad 20px；chip 統一 min-height 34px／padding 0 14px，未選中邊框以 color-mix 55% 淡化；排序列從三個平鋪文字改成單一「排序：即將出發 ▼」下拉（.sort-btn / .sort-menu / .sort-scrim，Icon 新增 chevronDown），點擊才展開三個選項；「4 趟」改「共 4 趟」；旅程卡 min-height big 230→216、small 190→178（約 6%），cover 漸層底部加深到 .88 保證白字對比，名稱 21/19px 為第一層、地點 12px opacity .84、日期天數 12px opacity .76、預算 11.5px 為第四層；狀態與倒數 badge 統一為 .badge（高 24px、pill 圓角、padding 0 10px），規劃中改半透明玻璃底、倒數改白底粗體提高權重，移除 .countdown-pill；收藏鈕統一 36px／blur 10px，收藏後改白底＋品牌橘實心星；預算列改「已花 AUD X / Y」＋「剩餘 AUD Z」（超支顯示「超支」並轉警示色），bar 高 7→6px，配色改成 <85% 綠、85–100% 琥珀、>100% 紅（照片上用加亮版 #8FD9AE / #F5C563 / #FF9270）；tokens 新增 --nav-safe（safe-area 只取七成）與 --nav-total，--nav-h 64→58，bottomnav icon 與文字 gap 4→1、文字 11.5px、navitem 改 flex:1 1 0 等分；.scroll 新增 .has-fab 變體多留 96px，捲到底最後一張卡不再被 FAB／底部導覽擋住；區塊間距統一為 Header→Search 12、Search→Filter 16、Filter→Sort 12、Sort→Section title 24、Title→Card 12、Card→Card 12。npm run build 通過
- 為什麼：使用者要求降低 Header 與控制區域佔用高度、強化卡片資訊層級，讓同一螢幕能看到更多旅程，同時保留原本米白＋橘色的旅行感視覺
...
## 2026-07-21 10:21（v1.21）
- 範圍：src/screens/BudgetScreen.jsx、CompanionsScreen.jsx、ItineraryScreen.jsx、LogisticsScreen.jsx、PrepScreen.jsx、SummaryScreen.jsx、HomeScreen.jsx、TripsScreen.jsx
- 做了什麼：幣別顯示規則推及全 App，分三類處理。
  1. 單一旅程的頁面（預算、分帳、行程、交通住宿、行前準備、旅程總結）：幣別統一標在 topbar 的 greeting（如「京都 · ¥ JPY」），內文金額一律改為純數字。
  2. 首頁與旅程列表：這兩頁同時列出多趟、可能不同幣別的旅程，若只標一次會無法分辨，故改為每張卡片標一次幣別代碼（如「JPY 已花 12,000　剩 8,000」），而非每個金額都加前綴。
  3. 會離開 App 的內容維持完整幣別符號：旅程總結的分享文字、PDF 匯出、以及標示為「可分享」的報告卡——截圖分享出去若沒有幣別會造成誤解。已停用的 AI 文案（aiMock）也維持原樣。
  另保留記帳頁與預算頁的「≈ NT$」台幣換算符號（那是另一種幣別）。全檔掃描確認無殘留。build 驗證通過
- 為什麼：使用者要求整個 App 統一幣別顯示方式

## 2026-07-21 10:26（v1.22）
